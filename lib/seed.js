/**
 * Shared demo seed for CLI (`bun run seed`) and /manage one-click seed.
 * Replaces the testimonials list in KV and pins a starter featured set.
 * Does not create UploadThing video files — seeds use images, YouTube, and X only.
 */
import {
  createId,
  kvConfigured,
...
/** Indices in SEED_TESTIMONIALS to pin for /embed (varied media mix). */
export const SEED_FEATURED_INDEXES = [1, 3, 5, 0];

export function buildSeedItems(now = Date.now()) {
  const items = [];
  for (let i = 0; i < SEED_TESTIMONIALS.length; i++) {
    const seed = SEED_TESTIMONIALS[i];
    const validated = validateTestimonial(seed);
    if (validated.error) {
      return { error: `Seed ${i} invalid: ${validated.error}` };
    }
    items.push({
      id: createId(),
      name: validated.name,
...
/**
 * Replace testimonials in KV and pin a starter featured set.
 * @returns {{ count: number, featured: number } | { error: string }}
 */
export async function seedDemoTestimonials() {
  if (!kvConfigured()) {
    return { error: "KV is not configured. Set KV_REST_API_URL and KV_REST_API_TOKEN." };
  }

  const built = buildSeedItems();
  if (built.error) return { error: built.error };

  const { items } = built;
  await saveTestimonials(items);

  const featured = SEED_FEATURED_INDEXES.map((index) => items[index].id);
  const result = await setFeaturedIds(featured);
  if (result.error) return { error: result.error };

  return { count: items.length, featured: result.ids.length };
}
